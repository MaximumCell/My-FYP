...
const SimulationPage = () => {
  ...
  const handleRunSimulation = async () => {
    ...
    try {
      ...
      const response = await axios.post(`${API}/simulation/simulation`, {
      ...
      setTimeout(() => {
        if (response.data.plot_url) {
          // Use API base and a timestamp to prevent browser caching
          setPlotUrl(
            `${API}/simulation/download_plot?t=${new Date().getTime()}`
          );
        }
        setLoading(false);
      }, 1000);
  ...
  const buttonClass =
    "bg-blue-600 hover:bg-blue-700 text-white font-semibold px-6 py-2 rounded-md shadow transition duration-300 flex items-center justify-center";
    ...
    <div className={containerClass}>
      ...
      {errorMessage && (
        <div
          ...
          } text-red-500 p-4 rounded-lg w-full max-w-4xl mb-6 border`}
        >

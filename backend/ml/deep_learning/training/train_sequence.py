"""Training wrapper for sequence models with basic validation and timestamped saving."""
import os
import time
try:
...
from ..models.sequence import get_model
...
def train_model(csv_path, target_column=None, timesteps=10, epochs=5, batch_size=32, config=None, model_out_path=None):
    ...
    # Use numeric columns only for sequence input
    ...
    if model_out_path is None:
        ts = int(time.time())
        model_out_path = os.path.join('trained_models', f'deep_sequence_{ts}')
